# Neovim Setup Guide — macOS (Apple Silicon)
...
```bash
xcode-select --install                        # build tools: git, clang, make
...
```bash
# install build dependencies first or pyenv install will fail
...
> SDKMAN requires Bash 4+. macOS ships with Bash 3.2 and the installer will hard-fail.
> Install a modern Bash first:
> ```bash
> brew install bash
> ```
...
> **Where dotnet actually lives:** Homebrew installs dotnet to `/usr/local/share/dotnet/dotnet`,
> NOT `/usr/local/bin/dotnet`. This matters for DAP — see the `lang-csharp.lua` section below.
...
return {
  ...
  },
  {
    ...
    config = function()
      local dap = require("dap")
      -- netcoredbg: mason ships x86_64-only for macOS; arm64 requires a source build.
      -- vsdbg (Microsoft) is not an option — it enforces a license check at configurationDone
      -- that blocks any client that isn't Microsoft's own VS Code extension.
      dap.adapters.coreclr = {
        type = "executable",
        command = vim.fn.expand("~/.local/share/nvim/mason/packages/netcoredbg/libexec/netcoredbg/netcoredbg"),
        args = { "--interpreter=vscode" },
        -- dotnet lives at /usr/local/share/dotnet on macOS, not /usr/local/bin
        env = {
          PATH = "/usr/local/share/dotnet:/usr/local/bin:/usr/bin:/bin",
...
#### Apple Silicon: netcoredbg must be built from source

Mason's registry ships `netcoredbg-osx-amd64.tar.gz` for **both** `darwin_x64` and `darwin_arm64` targets. On Apple Silicon that x86_64 binary runs under Rosetta 2, and it crashes immediately when it tries to inject its debug interface into a native arm64 .NET process (SIGSEGV in `ManagedDebuggerHelpers::StartupCallback`).

**Do not bother with vsdbg.** Microsoft's debugger enforces a license check at `configurationDone` that refuses to work with anything other than the official VS Code C# extension, regardless of what flags or environment variables you set.

**Fix: build netcoredbg natively for arm64.**

```bash
# Prerequisites (all already present if you followed this guide)
#   cmake (brew install cmake if missing)
#   clang (Xcode Command Line Tools)
#   dotnet SDK

git clone --depth=1 https://github.com/Samsung/netcoredbg.git /tmp/netcoredbg-src
mkdir /tmp/netcoredbg-src/build && cd /tmp/netcoredbg-src/build

cmake .. \
  -DCMAKE_BUILD_TYPE=Release \
  -DDOTNET_DIR=/usr/local/share/dotnet \
  -DCORECLR_BRANCH=release/10.0 \
  -DDOTNET_CHANNEL=10.0

make -j$(sysctl -n hw.logicalcpu)
```

cmake configure takes ~30 s (shallow-clones `dotnet/runtime` for headers). The build itself takes ~5 min.

Now replace the mason binary with the arm64 one:

```bash
MASON_DIR="$HOME/.local/share/nvim/mason/packages/netcoredbg/libexec/netcoredbg"
BUILD_DIR="/tmp/netcoredbg-src/build/src"

# Back up the x86_64 originals
for f in netcoredbg libdbgshim.dylib ManagedPart.dll; do
  cp "$MASON_DIR/$f" "$MASON_DIR/$f.x86_64.bak"
done

# Install arm64 artifacts
cp "$BUILD_DIR/netcoredbg"          "$MASON_DIR/"
cp "$BUILD_DIR/libdbgshim.dylib"    "$MASON_DIR/"
cp "$BUILD_DIR/ManagedPart.dll"     "$MASON_DIR/"
cp "$BUILD_DIR/Microsoft.CodeAnalysis"*.dll "$MASON_DIR/"

# Verify
file "$MASON_DIR/netcoredbg"
# → Mach-O 64-bit executable arm64  ✓
```

> **After a `MasonUpdate`** that touches netcoredbg, the x86_64 binary will be reinstalled.
> Re-run the copy step above (the `/tmp/netcoredbg-src/build` artefacts are still there
...
**After MasonInstall completes, rebuild netcoredbg for arm64** — see the Apple Silicon section above.
...
| `netcoredbg` | .NET DAP adapter (requires arm64 rebuild — see above) |
...
vsdbg is Microsoft's debugger, bundled with VS Code's C# extension. It performs a license
check at `configurationDone` that rejects any DAP client that isn't the official VS Code
C# extension — no flags, no environment variables, no workarounds. netcoredbg (Samsung,
MIT license) is the correct tool for nvim-dap.

### Why arm64 netcoredbg from source?

Mason's registry maps both `darwin_arm64` and `darwin_x64` to the same `netcoredbg-osx-amd64.tar.gz`.
On Apple Silicon that x86_64 binary runs under Rosetta 2. The crash happens in
`ManagedDebuggerHelpers::StartupCallback` because Rosetta can't cross-inject a debug
interface from an x86_64 process into a native arm64 .NET process. Building from source
with the host's native toolchain produces an arm64 binary that works correctly.
